...
@Component({
  selector: 'app-nivel-tres-2',
  templateUrl: './nivel-tres.2.page.html',
  styleUrls: ['./nivel-tres.2.page.scss'],
})
export class NivelTres2Page {
  public progress = 0;
  ...
  async verificarRespuesta(respuesta: string) {
    ...
    if (respuesta === respuestaCorrecta) {
      this.resultadoMensaje = '¡Correcto! Has presentado correctamente.';
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [{
        ...
        handler: () => {
          ...
          this.router.navigate(['/nivel-tres.3']);
        }
      }]
    });
    await alert.present();
