...
@Component({
  selector: 'app-nivel-tres-4',
  templateUrl: './nivel-tres.4.page.html',
  styleUrls: ['./nivel-tres.4.page.scss'],
})
export class NivelTres4Page {
  public progress = 0;
  ...
  async verificarRespuesta(respuesta: string) {
    const respuestaCorrecta = 'A'; // Cambia esto a la respuesta correcta

    this.respuestaDada = true;

    if (respuesta === respuestaCorrecta) {
      this.resultadoMensaje = '¡Correcto! Has agradecido correctamente.';
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [{
        ...
        handler: () => {
          ...
          this.router.navigate(['/nivel-tres.5']);
        }
      }]
    });
    await alert.present();
