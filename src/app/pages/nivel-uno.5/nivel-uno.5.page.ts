...
@Component({
  ...
  templateUrl: './nivel-uno.5.page.html',
  styleUrls: ['./nivel-uno.5.page.scss'],
})
export class NivelUno5Page implements OnInit {
  public progress = 0;
  ...
  async handleCorrectAnswer(idsubnivel: number) {
    ...
    if (subnivel && !subnivel.completado) {
      ...
      if (this.correctAnswers >= this.totalLevels) {
        ...
        const nextSubnivel = `nivel-uno.${this.correctAnswers + 6}`; // Ajustar la navegación
        this.router.navigate([`/${nextSubnivel}`]);
      }
    } else {
      // Navegar al siguiente subnivel si ya está completo
      const nextSubnivel = `nivel-uno.${this.correctAnswers + 6}`; // Ajusta la navegación
      this.router.navigate([`/${nextSubnivel}`]);
    }
  }
