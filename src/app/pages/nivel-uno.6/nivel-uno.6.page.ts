...
@Component({
  selector: 'app-nivel-uno-6',
  templateUrl: './nivel-uno.6.page.html',
  styleUrls: ['./nivel-uno.6.page.scss'],
})
export class NivelUno6Page implements OnInit {
  public progress = 0;
  ...
  async handleCorrectAnswer(idsubnivel: number) {
    ...
    if (subnivel && !subnivel.completado) {
      ...
      if (this.correctAnswers >= this.totalLevels) {
        ...
        const nextSubnivel = `nivel-uno.${this.correctAnswers + 7}`; // Ajustar la navegación
        this.router.navigate([`/${nextSubnivel}`]);
      }
    } else {
      // Navegar al siguiente subnivel si ya está completo
      const nextSubnivel = `nivel-uno.${this.correctAnswers + 7}`; // Ajusta la navegación
      this.router.navigate([`/${nextSubnivel}`]);
    }
  }
