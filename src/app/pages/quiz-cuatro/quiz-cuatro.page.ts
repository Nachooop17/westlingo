...
@Component({
  selector: 'app-quiz-cuatro',
  templateUrl: './quiz-cuatro.page.html',
  styleUrls: ['./quiz-cuatro.page.scss'],
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
...
export class QuizCuatroPage implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    console.log('QuizCuatroPage: Componente cargado');
    this.route.paramMap.subscribe(params => {
      ...
      if (levelIdParam && subnivelIdParam) {
        ...
        console.log(`QuizCuatroPage: Recibido levelId: ${this.levelId}, subnivelId: ${this.subnivelId}`);
      } else {
        console.error('QuizCuatroPage: ID de nivel o subnivel no proporcionado.');
    ...
    this.authService.currentUser$.subscribe(user => {
      ...
      if (!user) {
        console.log('QuizCuatroPage: No hay usuario, redirigiendo a login...');
        this.router.navigate(['/login']);
      }
    });

    console.log('QuizCuatroPage: Verificando videoElement:', this.videoElement);
    if (this.videoElement && this.videoElement.nativeElement) {
      console.log('QuizCuatroPage: videoElement.nativeElement:', this.videoElement.nativeElement);
      this.gestureService.initialize(this.videoElement.nativeElement, this.onResults.bind(this), () => {});
      console.log('QuizCuatroPage: GestureService inicializado.');
    } else {
      console.error('QuizCuatroPage: videoElement no está disponible en ngOnInit.');
    }
  }

  ngOnDestroy() {
    console.log('QuizCuatroPage: Componente destruido. Deteniendo GestureService.');
  ...
  onResults(results: any) {
    this.ngZone.run(() => {
      ...
      if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
        ...
        // Aquí adaptarías la lógica para el gesto del Quiz 4
        if (this.detectarGestoBebidaAlcoholica(landmarks)) { // REEMPLAZAR con la detección del nuevo gesto
          if (this.gestureDetected !== 'Bebida Alcohólica') { // REEMPLAZAR con el nombre del nuevo gesto
            this.gestureDetected = 'Bebida Alcohólica'; // REEMPLAZAR
            console.log('QuizCuatroPage: GESTO DETECTADO: Bebida Alcohólica'); // REEMPLAZAR
            if (!this.quizCompleted) {
              this.generarFeedbackGesto('Bebida Alcohólica'); // REEMPLAZAR
              this.submitQuiz();
            }
          }
        }
  ...
  // *** IMPORTANTE: Debes reemplazar esta lógica con la detección del gesto específico para el Quiz 4 ***
